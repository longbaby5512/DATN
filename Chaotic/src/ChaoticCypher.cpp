...
void ChaoticCypher::init(ChaoticCypher::Mode mode, const bytes& key) {
    ...
    key1 = ChaoticMap::generateKey(permType, bytes(hashKey.cbegin(), hashKey.cbegin() + static_cast<ptrdiff_t>(hashKey.size() / 2)));
    key2 = ChaoticMap::generateKey(subType, bytes(hashKey.cbegin() + static_cast<ptrdiff_t>(hashKey.size() / 2), hashKey.cend()));
